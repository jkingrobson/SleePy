...
| M10 period | The most active consecutive 10-hour period. From which both the activity ($M10_{activity}$) and time ($M10_{time}$) can be derived. | $\mathbf{M10} = \max(x_{10})$ | 5 |
| L5 period | The least active consecutive 5-hour period. From which both the activity ($L5_{activity}$) and time ($L5_{time}$) can be derived. | $\mathbf{L5} = \min(x_{5})$ | 5 |
